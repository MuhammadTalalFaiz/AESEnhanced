...
def key_expansion(key):
    ...
    while len(round_keys) < 44:
        ...
        if len(round_keys) % 4 == 0:
            
            prev_key = g(prev_key, len(round_keys) // 4)
        new_key = [prev_key[i] ^ round_keys[-4][i] for i in range(4)]
...
def aes_encrypt(plaintext, key):
    ...
    # Final round (no mix columns step)
    state = substitute_bytes(state)
    state = shift_rows(state)
...
def main():
    ...
    plaintext = 'I love paladins'
    key = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x97, 0x21, 0x09, 0x9d, 0x8f, 0x62]  # Example key
    round_keys=key_expansion(key)
    deduce_key(round_keys)

    # # Encrypt the plaintext
    # encrypted_state = aes_encrypt(plaintext.encode(), key)

    
    # decrypted_state = aes_decrypt([byte for sublist in encrypted_state for byte in sublist], key)
